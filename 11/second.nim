...
proc find_window(serial_number: int): tuple[p: int, x: int, y: int, s: int] =
  var
  ...
  var
    max_power = low(int)
    (max_x, max_y, size) = (-1, -1, -1)

  for x in 1..width - window:
    for y in 1..height - window:
      for s in 1..min(width-x, height-y):
        var power = 0
        for ox in x..x+s - 1:
          for oy in y..y+s - 1:
            power += grid[ox][oy]
  ...
  return (max_power, max_x, max_y, size)

doAssert(find_window(18) == (113, 90, 269, 16), $find_window(18))
doAssert(find_window(42) == (119, 232, 251, 12), $find_window(42))
let result = find_window(7315)
echo($result)
echo($result.x, ',', $result.y, ',', $result.s)
